...
public class MatchMapper {
    ...
    public ChampionMatchDto toChampionMatchDto(MatchDto match, Integer championId) {
        ChampionMatchDto dto = new ChampionMatchDto();

        dto.setMatchId(match.getMatchId());
        dto.setMetadata(match.getMetadata());
        dto.setInfo(match.getInfo());

        boolean win = dto.getInfo().getParticipants().stream()
                .anyMatch(p -> p.getChampionId().equals(championId) && p.getWin());
        dto.setWin(win);

        ParticipantDto player = dto.getInfo().getParticipants().stream()
                .filter(p -> p.getChampionId().equals(championId))
                .findFirst().orElse(null);
        dto.setPlayer(player);
        ...
        List<ChampionImgInfoDto> imgs = dto.getInfo().getParticipants().stream()
                .map(p -> new ChampionImgInfoDto(baseImageUrl + p.getChampionId(), p.getChampionId().equals(championId)))
                .toList();
        dto.setParticipantChampionImgList(imgs);
